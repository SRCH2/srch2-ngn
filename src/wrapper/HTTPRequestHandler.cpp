...
void HTTPRequestHandler::updateCommand(evhttp_request *req, Srch2Server *server)
{
    /* Yes, we are expecting a post request */
    switch (req->type)
    {
...
void HTTPRequestHandler::writeCommand_v1(evhttp_request *req, Srch2Server *server)
{
    /* Yes, we are expecting a post request */
    switch (req->type)
    {
...
void HTTPRequestHandler::activateCommand(evhttp_request *req, Srch2Server *server)
{
    /* Yes, we are expecting a post request */
    switch (req->type)
    {
...
void HTTPRequestHandler::infoCommand(evhttp_request *req, Srch2Server *server, const string &versioninfo)
{
    evkeyvalq headers;
    evhttp_parse_query(req->uri, &headers);
...
void HTTPRequestHandler::lookupCommand(evhttp_request *req, Srch2Server *server)
{
    evkeyvalq headers;
    evhttp_parse_query(req->uri, &headers);
...
void HTTPRequestHandler::searchCommand(evhttp_request *req, Srch2Server *server)
{
    ...
    if(!isValid){
    	 ...
    	 */
    	bmhelper_evhttp_send_reply(req, HTTP_BADREQUEST, "Bad Request", paramContainer.getMessageString(), headers);
    }
