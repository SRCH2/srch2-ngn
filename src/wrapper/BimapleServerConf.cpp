...
// $Id: BimapleServerConf.cpp 3429 2013-06-10 09:13:54Z jiaying $
...
BimapleServerConf::BimapleServerConf(int argc, char** argv, bool &configSuccess, std::stringstream &parseError)
{
	...
	config.add_options()
				...
				("license-file", po::value<string>(), "File name with path to the bimaple license key file") // REQUIRED
...
void BimapleServerConf::kafkaOptionsParse(const po::variables_map &vm, bool &configSuccess, std::stringstream &parseError)
{
...
void BimapleServerConf::_setDefaultSearchableAttributeBoosts(const vector<string> &searchableAttributesVector)
{
...
void BimapleServerConf::parse(const po::variables_map &vm, bool &configSuccess, std::stringstream &parseError)
{
	...
	if ( vm.count("attributes-sort") && (vm["attributes-sort"].as<string>().compare(ignoreOption) != 0) )
    {
			...
			&& vm.count("attributes-sort-default-value") && (vm["attributes-sort-default-value"].as<string>().compare(ignoreOption) != 0) )
	    {
		    ...
		    vector<bimaple::instantsearch::FilterType> attributeTypes;
...

		    {
			    ...
			    for(unsigned iter=0; iter<attributeTypesTmp.size(); iter++)
			    {
				    if (attributeTypesTmp[iter].compare("0") == 0)
					    attributeTypes.push_back(bimaple::instantsearch::UNSIGNED);
				    else if (attributeTypesTmp[iter].compare("1") == 0)
					    attributeTypes.push_back(bimaple::instantsearch::FLOAT);
			    }
		    }
...
BimapleServerConf::~BimapleServerConf()
{

}

const std::string& BimapleServerConf::getCustomerName() const
{
	return kafkaConsumerTopicName;
}

uint32_t BimapleServerConf::getDocumentLimit() const
{
	return documentLimit;
}

uint64_t BimapleServerConf::getMemoryLimit() const
{
	return memoryLimit;
}

uint32_t BimapleServerConf::getMergeEveryNSeconds() const
{
	return mergeEveryNSeconds;
}

uint32_t BimapleServerConf::getMergeEveryMWrites() const
{
	return mergeEveryMWrites;
}

int BimapleServerConf::getIndexType() const
{
	return indexType;
}

const string& BimapleServerConf::getAttributeLatitude() const
{
	return attributeLatitude;
}

const string& BimapleServerConf::getAttributeLongitude() const
{
	return attributeLongitude;
}

float BimapleServerConf::getDefaultSpatialQueryBoundingBox() const
{
	return defaultSpatialQueryBoundingBox;
}

int BimapleServerConf::getNumberOfThreads() const
{
	return numberOfThreads;
}

DataSourceType BimapleServerConf::getDataSourceType() const
{
	return dataSourceType;
}

IndexCreateOrLoad BimapleServerConf::getIndexCreateOrLoad() const
{
	return indexCreateOrLoad;
}

WriteApiType BimapleServerConf::getWriteApiType() const
{
	return writeApiType;
}

const string& BimapleServerConf::getIndexPath() const
{
	return indexPath;
}

const string& BimapleServerConf::getFilePath() const
{
	return this->filePath;
}

const string& BimapleServerConf::getPrimaryKey() const
{
	return primaryKey;
}

const map<string, pair<unsigned, unsigned> > * BimapleServerConf::getSearchableAttributes() const
{
	return &searchableAttributesTriple;
}

const vector<string> * BimapleServerConf::getAttributesToReturnName() const
{
	return &attributesToReturn;
}

const vector<string> * BimapleServerConf::getSortableAttributesName() const
{
	return &sortableAttributes;
}

const vector<bimaple::instantsearch::FilterType>  * BimapleServerConf::getSortableAttributesType() const
{
	return &sortableAttributesType;
}

const vector<string> * BimapleServerConf::getSortableAttributesDefaultValue() const
{
	return &sortableAttributesDefaultValue;
}


/*const vector<unsigned> * BimapleServerConf::getAttributesBoosts() const
{
	return &attributesBoosts;
}*/

const string& BimapleServerConf::getAttributeRecordBoostName() const
{
...
const std::string& BimapleServerConf::getScoringExpressionString() const
{
	return scoringExpressionString;
}

int BimapleServerConf::getSearchResponseJSONFormat() const
{
	return searchResponseJsonFormat;
}

const string& BimapleServerConf::getRecordAllowedSpecialCharacters() const
{
	return allowedRecordTokenizerCharacters;
}

int BimapleServerConf::getSearchType() const
{
	return searchType;
}

int BimapleServerConf::getIsPrimSearchable() const
{
	return isPrimSearchable;
}

bool BimapleServerConf::getIsFuzzyTermsQuery() const
{
	return exactFuzzy;
}

bool BimapleServerConf::getQueryTermType() const
{
	return queryTermType;
}

unsigned BimapleServerConf::getQueryTermBoost() const
{
	return queryTermBoost;
}

float BimapleServerConf::getQueryTermSimilarityBoost() const
{
	return queryTermSimilarityBoost;
}

float BimapleServerConf::getQueryTermLengthBoost() const
{
	return queryTermLengthBoost;
}

float BimapleServerConf::getPrefixMatchPenalty() const
{
    return prefixMatchPenalty;
}

bool BimapleServerConf::getSupportAttributeBasedSearch() const
{
	return supportAttributeBasedSearch;
}

int BimapleServerConf::getSearchResponseFormat() const
{
	return searchResponseFormat;
}

const string& BimapleServerConf::getAttributeStringForMySQLQuery() const
{
	return attributeStringForMySQLQuery;
}

const string& BimapleServerConf::getLicenseKeyFileName() const
{
	return licenseKeyFile;
}

const std::string& BimapleServerConf::getTrieBootstrapDictFileName() const
{
	return this->trieBootstrapDictFile;
}

const string& BimapleServerConf::getHTTPServerListeningHostname() const
{
	return httpServerListeningHostname;
}

const string& BimapleServerConf::getHTTPServerListeningPort() const
{
	return httpServerListeningPort;
}

const string& BimapleServerConf::getKafkaBrokerHostName() const
{
	return kafkaBrokerHostName;
}

uint16_t BimapleServerConf::getKafkaBrokerPort() const
{
	return kafkaBrokerPort;
}

const string& BimapleServerConf::getKafkaConsumerTopicName() const
{
	return kafkaConsumerTopicName;
}

uint32_t BimapleServerConf::getKafkaConsumerPartitionId() const
{
	return kafkaConsumerPartitionId;
}

uint32_t BimapleServerConf::getWriteReadBufferInBytes() const
{
	return writeReadBufferInBytes;
}

uint32_t BimapleServerConf::getPingKafkaBrokerEveryNSeconds() const
{
	return pingKafkaBrokerEveryNSeconds;
}

int BimapleServerConf::getDefaultResultsToRetrieve() const
{
	return resultsToRetrieve;
}

int BimapleServerConf::getAttributeToSort() const
{
	return attributeToSort;
}

int BimapleServerConf::getOrdering() const
{
	return ordering;
}

bool BimapleServerConf::isRecordBoostAttributeSet() const
{
	return recordBoostAttributeSet;
}

const string& BimapleServerConf::getHTTPServerAccessLogFile() const
{
	return httpServerAccessLogFile;
}

const string& BimapleServerConf::getHTTPServerErrorLogFile() const
{
	return httpServerErrorLogFile;
}

unsigned BimapleServerConf::getCacheSizeInBytes() const
{
