...
bool MergeByShortestListOperator::open(QueryEvaluatorInternal * queryEvaluator, PhysicalPlanExecutionParameters & params){
	...
	// Assert because no operator is expected to send cache entry down to MergeByShortestListOperator
	...
	if(this->queryEvaluator != NULL && // this is for CTEST ShortestList_Test, in normal cases, queryEvaluator cannot be NULL
		...
		for(unsigned childOffset = 0 ; childOffset != this->getPhysicalPlanOptimizationNode()->getChildrenCount() ; ++childOffset){
			...
			if(childOffset >= mergeShortestCacheEntry->children.size()){
				...
				// We are sending the cached object (and not a clone) down to children.
	...
	// To make sure this pointer doesn't pass a wrong address to anywhere else.
	params.cacheObject = NULL;
...
bool MergeByShortestListOperator::close(PhysicalPlanExecutionParameters & params){
	...
	// close the children
	params.parentIsCacheEnabled = true;
	// Collecting possible new cache entries that can be generated by children
	// and making a new cache entry of type MergeByShortestListCacheEntry
	params.cacheObject = NULL;
	for(unsigned childOffset = 0 ; childOffset != this->getPhysicalPlanOptimizationNode()->getChildrenCount() ; ++childOffset){
	...
	// cache
	// Note: Cache entry of this operator is not passed up to parent because
	// our caching architecture must have proper implementation for specific plans.
