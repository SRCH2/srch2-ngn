...
int SynonymFilter::getTokenTypeOf(const std::string &prefixToken) {
	std::map<std::string, pair<SynonymTokenType, std::string> >::const_iterator pos = this->synonymMap.find(prefixToken);
	if (pos != this->synonymMap.end()) {
		return this->synonymMap[prefixToken].first ;
	} else {
		return SYNONYM_NOT_PREFIX_NOT_COMPLETE;
	}
}

const string SynonymFilter::getSynonymOf(const std::string &word) {
	// finds the subWord in the map
	std::map<std::string, pair<SynonymTokenType, std::string> >::const_iterator pos = this->synonymMap.find(word);
	if (pos != this->synonymMap.end()) {
		return this->synonymMap[word].second;
	}
	return "";
}
...
bool SynonymFilter::incrementToken() {
	while (true) {
		...
		int synonymTokenTypeOfCurrentToken =	this->getTokenTypeOf(currentToken);
		...
		if (this->tokenBuffer.empty()) {
			...
			if (synonymTokenTypeOfCurrentToken == SYNONYM_NOT_PREFIX_NOT_COMPLETE) {
				this->emitBuffer.push_back(currentToken);
				this->emitCurrentToken();
				return true;
			// if there is one synonym match for the new token
			} else if (synonymTokenTypeOfCurrentToken == SYNONYM_COMPLETE_ONLY) {
				if (this->keepOriginFlag == SYNONYM_KEEP_ORIGIN) { // checks the flag of Keeping origin word
		...
		// if the buffer is not empty
			...
			int synonymTokenTypeOfPreviousFollowedByCurrentToken = this->getTokenTypeOf(previousTokens + currentToken);
			// if there is NOT any match for the elements in the buffer that followed by current token

			if ( synonymTokenTypeOfPreviousFollowedByCurrentToken == SYNONYM_NOT_PREFIX_NOT_COMPLETE) {
				pushSynonymsOfExistingTokensInEmitBuffer();
				if (synonymTokenTypeOfCurrentToken == SYNONYM_NOT_PREFIX_NOT_COMPLETE) {
					this->emitBuffer.push_back(currentToken);
				} else if (synonymTokenTypeOfCurrentToken == SYNONYM_COMPLETE_ONLY) {
			...
			} else if (synonymTokenTypeOfPreviousFollowedByCurrentToken == SYNONYM_COMPLETE_ONLY) {
				/*
