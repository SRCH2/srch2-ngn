// $Id: chat.cpp 1217 2011-06-15 20:59:17Z bimaple.vijay $
/*
...
using bmhttp::BimapleServer;
using bmhttp::HTTPResponse;
using bmhttp::BimapleServerConf;
using bmhttp::BimapleServerLogger;
...
void cb_bmsearch(evhttp_request *req, void *arg) {
	...
	BimapleServer *server = reinterpret_cast<BimapleServer *>(arg);
	evhttp_add_header(req->output_headers, "Content-Type",
                    "application/json; charset=UTF-8");
	bmhttp::HTTPResponse::searchCommand(req, server);
}
...
void cb_bmlookup(evhttp_request *req, void *arg) {
	...
	evhttp_add_header(req->output_headers, "Content-Type",
                    "application/json; charset=UTF-8");
	bmhttp::HTTPResponse::lookupCommand(req, server);
}
...
{		
	...
	BimapleServerConf *serverConf = new BimapleServerConf(argc, argv, parseSuccess, parseError);
    BimapleServerLogger *serverLogger = new BimapleServerLogger(serverConf->getHTTPServerAccessLogFile());
	...
	//cout << "bimaple server started." << endl;
	...
	//std::cout << "Started Bimaple server:" << http_addr << ":" << http_port << std::endl;
    ...
    if (server.indexDataContainerConf->getWriteApiType() == bmhttp::HTTPWRITEAPI)
    {
