...
void ConfigManager::parseDataFieldSettings(const xml_node &parentNode, CoreInfo_t *coreInfo, bool &configSuccess, std::stringstream &parseError, std::stringstream &parseWarnings)
{
    ...
    if (childNode) {
        parseSchemaRefactored(childNode, &coreParseState, coreInfo, configSuccess, parseError, parseWarnings);
        if (configSuccess == false) {
            return;
        }
    }
	    ...
	    if (fieldsNode) {
	        for (xml_node field = fieldsNode.first_child(); field; field = field.next_sibling()) {
	            if (string(field.name()).compare(fieldString) == 0) {
	                ...
	                setDefaultFlags(isMultiValued, isSearchable, isRefining, isHighlightEnabled);
	                cout << field.attribute("name").value() << "\n";
	                cout << field.attribute("refining").value() << "\n";
	                cout << field.attribute("type").value() << "\n";
	                cout << field.attribute("indexed").value() << "\n";
	                cout << field.attribute("default").value() << "\n";
	                cout << field.attribute("required").value() << "\n";
	                if(!setFlagsFromFile(field, isMultiValued, isSearchable, isRefining, isHighlightEnabled, parseError, configSuccess)){
	                	configSuccess = false;
	                	return;
	                }
...
void ConfigManager::setDefaultFlags(bool &isMultivalued, bool &isSearchable, bool &isRefining, bool &isHighlightEnabled){
	isMultivalued = false;
	isSearchable = false;
	isRefining = false;
...
bool ConfigManager::setSearchableRefiningFromIndexedAttribute(xml_node field, bool &isSearchable, bool &isRefining, std::stringstream &parseError, bool &configSuccess){
...
bool ConfigManager::setSearchableAndRefining(xml_node field, bool &isSearchable, bool &isRefining, std::stringstream &parseError, bool &configSuccess){
...
bool ConfigManager::setFlagsFromFile(xml_node field, bool &isMultiValued, bool &isSearchable, bool &isRefining, bool &isHighlightEnabled, std::stringstream &parseError, bool &configSuccess){
	string tempUse = "";
	...
	cout << field.attribute(indexedString).name() << flush <<"\n";
	cout << field.name() << "flush" << flush;
