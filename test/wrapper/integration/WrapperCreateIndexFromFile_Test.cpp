...
void test1(int argc, char** argv) {
    ...
    message << "IndexCreateOrLoad: " << serverConf->getIndexCreateOrLoad() << "\n";
    ...
    const map<string, pair<unsigned, unsigned> > * searchableAttributes = serverConf->getSearchableAttributes();
    map<string, pair<unsigned, unsigned> >::const_iterator iter;
    message << "Searchable Attributes:\n";
    for (iter = searchableAttributes->begin(); iter != searchableAttributes->end(); iter++) {
        message << iter->first << "  " << iter->second.first << "  " << iter->second.second << "\n";
    }
   ...
   const vector<string> * sortableAtt = serverConf->getSortableAttributesName() ;
   const vector<srch2::instantsearch::FilterType> * sortableAttTypes = serverConf->getSortableAttributesType() ;
   const vector<string> * sortableAttDefaultValues = serverConf->getSortableAttributesDefaultValue() ;
   message << "Sortable Attributes:\n";
   for (int i = 0; i < sortableAtt->size(); i++) {
       message << (*sortableAtt)[i] << "  ";
       message << (*sortableAttTypes)[i] << "  ";
       message << (*sortableAttDefaultValues)[i] << "  \n";
   }
