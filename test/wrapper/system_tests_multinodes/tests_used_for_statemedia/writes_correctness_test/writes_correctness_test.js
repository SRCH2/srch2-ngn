...
function search_request(queries_array) {
  return { method: 'GET'
           , uri: search_base_url + '?rows=' + limit + '&q=%7BdefaultPrefixComplete=COMPLETE%7D' + queries_array.join('~%20AND%20')+'*~'
         }
}
...
function search_callback(response, expect_success, expected_key) {
  ...
  if (!found) {
      failed++;
      //console.log('Failed ' + cursor + 'th search query. Expected ' + expected_key);
  }
...
function do_search(queries_array, expect_success, expected_key) {
    var encoded = [];
    for (var i = 0, j = 0; i < queries_array.length; i++) {
	if (queries_array[i].length > 0) {
	    encoded[j++] = encodeURIComponent(queries_array[i].replace(/[:^*~]/gi, ''));
	}
    }

    var response = make_request( search_request(encoded), search_callback, expect_success, expected_key );
}
