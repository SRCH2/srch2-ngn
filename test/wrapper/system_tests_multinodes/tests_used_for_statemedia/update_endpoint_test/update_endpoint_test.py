import sys, urllib2, json, time, os
...
response = urllib2.urlopen('http://localhost:' + port + '/search?q=%7BdefaultPrefixComplete=COMPLETE%7DHarry~%20AND%20Potter~%20AND%2012345*~').read()
...
if (len(response_json['results'])):
    ...
    print "ERROR: No results in response (" + response + ")"
    os._exit(1)
...
response = urllib2.urlopen('http://localhost:' + port + '/search?q=%7BdefaultPrefixComplete=COMPLETE%7D' + '~%20AND%20'.join(new_attr_value.split()) + '*~').read()
response_json = json.loads(response)
