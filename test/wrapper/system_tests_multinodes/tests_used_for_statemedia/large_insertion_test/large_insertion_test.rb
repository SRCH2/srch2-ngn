...
class Srch2Client
  ...
  def self.query( query, limit=10 )
    ...
    query = query.split().map! { |k| "#{k}" }.join("~%20AND%20") + "*~"
    #puts query
    get( '/search', :query => { :q => query, :start => 0, :rows => limit, :fuzzy => 'true', :searchType => 'TopK' } )
  end
...
query_arr = []
counter = 0
File.foreach( baseDir+'/data/update.json') { |data|
