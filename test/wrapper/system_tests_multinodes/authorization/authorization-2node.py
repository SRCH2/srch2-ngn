...
def testExactA1(queriesAndResultsPath, binary_path, authKey):
    ...
    args = [ binary_path, '--config-file=./authorization/conf-auth.xml']

    if confirmPortAvailable(portA, authKey) == False:
        print 'Port ' + str(portA) + ' already in use - aborting'
        return -1

    print 'starting engine: ' + args[0] + ' ' + args[1]
    serverHandle1 = test_lib.startServer(args)
    pingServer( portA, authKey)

    #Start the engine server (node B)
    args = [ binary_path, '--config-file=./authorization/conf-auth-B.xml']

    if confirmPortAvailable(portB, authKey) == False:
        print 'Port ' + str(portB) + ' already in use - aborting'
        return -1

    print 'starting engine: ' + args[0] + ' ' + args[1]
    serverHandle2 = test_lib.startServer(args)
    pingServer( portB, authKey)
    ...
    #construct the query
    query='http://localhost:' + portB + '/search?OAuth=' + authKey + '&'
