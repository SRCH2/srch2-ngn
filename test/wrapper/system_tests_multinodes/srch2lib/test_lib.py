...
def startServer(argList, isMultiNode = True , pingTimeout = 15, redirectFd = None):
    ...
    for i in range(1,len(argList)):
        ...
        serverHandleList.append(subprocess.Popen(startArgList, stdout=redirectFd , stderr= redirectFd))
#TODO need to be 1 sec later
        time.sleep(10)
    ...
    print 'Wait for ' + str(len(argList)*3) + ' sec to stabilize the nodes.'
    time.sleep(len(argList)*3)

    return serverHandleList

# For cluster mode, we can only insert the data by the restful api
def loadIntialData(dataFile, waitToMergeTime = 15):
    f = open(dataFile)
    records  = f.readlines()
    f.close()
    for rec in records:
        print '-------------------------------------------------'
        response = insertRequest(rec)
        print response

    printLog(' ********   waiting ' + str(waitToMergeTime) + ' sec for merge to finish ********* ')
    time.sleep(waitToMergeTime)
