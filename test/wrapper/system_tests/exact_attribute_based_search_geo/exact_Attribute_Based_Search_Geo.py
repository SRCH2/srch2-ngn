#using python exact_Attribute_Based_Search_Geo.py queriesAndResults.txt
...
def testExactAttributeBasedSearchGeo(queriesAndResultsPath):
	...
	for line in f_in:
		...
		for i in range(0, len(queryKeyword)):
		    if i == (len(queryKeyword)-1):
		        ...
		        query=query+queryKeyword[i]+'+'
		query=query+'&fuzzy=0&ct_lat='+str(queryGeo[1])+'&ct_lng='+str(queryGeo[0])+'&radius='+ str(radius)
		#print query
...
if __name__ == '__main__':    
	...
	testExactAttributeBasedSearchGeo(queriesAndResultsPath)
