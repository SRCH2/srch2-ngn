...
int main(int argc, char **argv)
{
...

    {
        ...
        for (vector<string>::iterator vectIter = file.begin(); vectIter!= file.end(); vectIter ++) {
            ...
            pingNormalQuery(analyzer, queryEvaluator,*vectIter,resultCount, vector<unsigned>(), true);
    ...
    clock_gettime(CLOCK_REALTIME, &tstart_d);
    {
        ...
        for (vector<string>::iterator vectIter = file.begin(); vectIter!= file.end(); vectIter ++) {
            ...
            unsigned resultCount = 10;
            std::cout << "[[" << *vectIter << "]]" << std::endl;
            doubleSearcherPing(analyzer, queryEvaluator,*vectIter,resultCount, 0, vector<unsigned>(), true);
