...
do
   ...
   valgrind --leak-check=full --track-origins=yes --error-limit=no --gen-suppressions=yes --log-file=$(_VALGRIND_LOG $__GROUP_NAME $i) $(_ENGINE_BINARY) --config=$(_CONF_FILE $i) > $(_CONSULE_LOG $__GROUP_NAME $i) &
   sleep 2
