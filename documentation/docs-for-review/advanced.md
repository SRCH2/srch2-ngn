...
##Highlighting
The SDK provide the customized highlighter to show the matching keywords.
It can be returned from an *Indexable.getHighlighter()* implementation.
A highlighter object set the pre and the post html tag to wrap the matching keywords. 

For example, for the keyword of "beaty ame", 
the search result for the movie with the title "Beaty American" will
produce the output of 
```
'&lt;Beaty &lt;font color="#FF0000"&gt;&lt;b&gt;Ame&lt;/b&gt;&lt;/font&gt;rican'
```
(or visually, //TODO).

To enable the highlighter feature, we need to enable it for the certain fields.
The following code shows how to enable the highlight on the field "title" and 
...
The highlight fields can be retrieved by using the constant value of 
...
##Custom Ranking
The SDK provide a *RecordBoostField* to determine each record's record score when computing the relevance of
search results. This field will always be float in type, and should be set
from 1 to 100. 

The following code tell the SDK to take the value of the "recordBoost" field from the record
