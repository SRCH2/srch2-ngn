 ...
 * {@link com.srch2.android.http.service.SRCH2Engine#searchAllIndexes(String)} is called). If there are no results for a given
 * search input, the map will be <b>non-null</b> but contain no values for each index name key.
 * <br><br>
 * This callback can be registered by calling {@link com.srch2.android.http.service.SRCH2Engine#setSearchResultsListener(SearchResultsListener)}
...
public interface SearchResultsListener {
     ...
     * of the
     * {@link Indexable#getIndexName()} implementation of each <code>Indexable</code>) mapped to an <code>ArrayList</code> of the parsed <code>JSONObject</code>
     * records. If the search that was performed is specific to a single index, this map will only contain one key--
     * the name of that index searched; otherwise the map will contain results for all the indexes (such as when
     * {@link com.srch2.android.http.service.SRCH2Engine#searchAllIndexes(String)} is called). If there are no results for a given
     * search input, the map will be <b>non-null</b> but contain no values for each index name key.
